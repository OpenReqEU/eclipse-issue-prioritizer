...
#nltk.data.path = [os.path.join(helper.APP_PATH, "corpora", "nltk_data", "corpora")]


def word_net_lemmatizer(requirements, lang="en"):
    _logger.info("Lemmatization for requirements' tokens")

    def lemma_via_patternlib(token, pos, lang="en"):
        # set([u'ADV', u'NOUN', u'ADP', u'PROPN', u'NUM', u'VERB', u'AUX', u'CONJ', u'ADJ'])
        """
        English classes:
        ...
        German classes:
         ...
         X  ADP: adposition
         X  ADV: adverb
         X  AUX: auxiliary verb
         X  CONJ: coordinating conjunction
            DET: determiner
            INTJ: interjection
         X  NOUN: noun
         X  NUM: numeral
        ...
        if lang == "en":
            if pos in ("NN", "NNPS"):
                return en.singularize(token).lower()
            elif pos in ("VBD", "VBG", "VBN", "VBP", "VBZ", "MD", "JJ"):
                return en.lemma(token).lower()
        else:
            if pos in ("NOUN"):
                # singularize noun
                return de.singularize(token)
            elif pos in ("VERB", "AUX", "JJ"):
                return de.lemma(token).lower()
        return token
    ...
    for requirement in requirements:
        ...
        new_tokens = []
        new_pos_tags = []
        for (idx, t) in enumerate(requirement.title_tokens):
            assert t == requirement.title_tokens_pos_tags[idx][0]
            lemmatized_token = lemma_via_patternlib(requirement.title_tokens_pos_tags[idx][0],
                                                    requirement.title_tokens_pos_tags[idx][1],
                                                    lang=lang)
            new_tokens += [lemmatized_token]
            new_pos_tags += [(lemmatized_token, requirement.title_tokens_pos_tags[idx][1])]

        requirement.title_tokens = new_tokens
        requirement.title_tokens_pos_tags = new_pos_tags

        new_tokens = []
        new_pos_tags = []
        for idx, t in enumerate(requirement.description_tokens):
            assert t == requirement.description_tokens_pos_tags[idx][0]
            lemmatized_token = lemma_via_patternlib(requirement.description_tokens_pos_tags[idx][0],
                                                    requirement.description_tokens_pos_tags[idx][1],
                                                    lang=lang)
            new_tokens += [lemmatized_token]
            new_pos_tags += [(lemmatized_token, requirement.description_tokens_pos_tags[idx][1])]

        requirement.description_tokens = new_tokens
        requirement.description_tokens_pos_tags = new_pos_tags
